...
class KeyWordsGetter:
    ...
    def get(self, text, collocation_size):
        return ", ".join(
            [t for t, s in sgrank(self.spm(text), ngrams=collocation_size)]
        )
...
class NeuralNetworkManager:
    def __init__(self):
        ...
        self.image_converter = StableDiffusionPipeline.from_pretrained(
            self.image_converter_model
        ).to(self.device)
    ...
    def convert_image(self, text, steps, height, width):
        return self.image_converter(
            prompt=text, num_inference_steps=steps, height=height, width=width
        ).images[0]

    def translate(self, text):
        batch = self.tokenizer(
            [text], truncation=True, max_length=60, return_tensors="pt"
        )
